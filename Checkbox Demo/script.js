...
for (let state of states) {
  ...
  opt.innerText = state;
  statesDropdown.appendChild(opt);
}
statesDropdown.addEventListener("change", () => {
  const main = document.querySelector(".stateCities");
  let stateSelected = statesDropdown.value;
  if (!document.getElementById("cities")) {
    const citiesDropdown = document.createElement("select");
    citiesDropdown.setAttribute("id", "cities");
    for (let c of cities) {
      if (c.state == stateSelected) {
        const opt = document.createElement("option");
        opt.setAttribute("value", c.city);
        opt.innerText = c.city;
        citiesDropdown.appendChild(opt);
      }
    }
    const opt1 = document.createElement("option");
    opt1.setAttribute("value", "----Select City----");
    opt1.setAttribute("selected", "selected");
    opt1.innerText = "----Select City----";
    citiesDropdown.appendChild(opt1);
    main.appendChild(citiesDropdown);
  } else {
    document.getElementById("cities").remove();
    const citiesDropdown = document.createElement("select");
    citiesDropdown.setAttribute("id", "cities");
    citiesDropdown.value = "** Select City **";
    for (let c of cities) {
      if (c.state == stateSelected) {
        const opt = document.createElement("option");
        opt.setAttribute("value", c.city);
